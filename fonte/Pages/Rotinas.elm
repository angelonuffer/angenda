        ...
        [ div [ class "space-y-2" ]
            ...
            , div [ class "flex items-center gap-1.5 flex-wrap" ]
                ...
                , if routine.recurrence == "Semanal" && not (List.isEmpty routine.selectedDays) then
                    ...
                    text ""
                , if routine.startDate /= "" || routine.endDate /= "" then
                    span [ class "text-xs font-medium text-slate-500 ml-2 bg-slate-100 px-2 py-0.5 rounded" ]
                        [ text <|
                            if routine.startDate /= "" && routine.endDate /= "" then
                                routine.startDate ++ " até " ++ routine.endDate
                            else if routine.startDate /= "" then
                                "A partir de " ++ routine.startDate
                            else
                                "Até " ++ routine.endDate
                        ]
                  else
          ...
          div [ class "bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4" ]
            [ Html.form [ onSubmit config.submitMsg, class "space-y-4" ]
                ...
                , div [ class "grid grid-cols-1 sm:grid-cols-2 gap-4" ]
                    [ div []
                        [ label [ for "new-routine-start", class "block text-sm font-semibold text-slate-700 mb-1" ] [ text "Início (Opcional)" ]
                        , input
                            [ type_ "date"
                            , id "new-routine-start"
                            , value model.routineStartDateInput
                            , onInput InputRoutineStartDate
                            , class "w-full border border-slate-300 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-red-500 text-slate-800"
                            ]
                            []
                        ]
                    , div []
                        [ label [ for "new-routine-end", class "block text-sm font-semibold text-slate-700 mb-1" ] [ text "Fim (Opcional)" ]
                        , input
                            [ type_ "date"
                            , id "new-routine-end"
                            , value model.routineEndDateInput
                            , onInput InputRoutineEndDate
                            , class "w-full border border-slate-300 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-red-500 text-slate-800"
                            ]
                            []
                        ]
